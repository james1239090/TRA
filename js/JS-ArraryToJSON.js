 ...
 var obj2 = {};
 var myTRCity = [];
 for (var i = 0; i < TRStation.length; i = i + 4) {
     if(TRStation[i]==="18"){
     var objStations = {};
     objStations.id = TRStation[i + 1];
     objStations.name = TRStation[i + 2];
     objStations.engName = TRStation[i + 3];
     arrSatations.push(objStations);
     }

 }

var objData = {};
for (var i=0;i<TRCity.length;i = i + 3){
	var objCity= {};
	objCity.city = TRCity[i+1];
	objCity.engCity = TRCity[i+2];
	objCity.stations = [];
	arrData.push(objCity);
}

  //console.log(JSON.stringify(arrSatations));
